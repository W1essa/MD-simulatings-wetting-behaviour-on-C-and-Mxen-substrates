...
def create_graphite_stack(base_layer, height, interlayer_dist, bond_length):
    ...
    layers_needed = int(np.ceil(height / interlayer_dist)) + 1
...
if __name__ == "__main__":
    ...
    # --- Edge treatment ---
    if STRUCTURE_TYPE == "pillars" and PASSIVATE_PILLARS:
        print("[INFO] Passivating pillar edges...")
        final_structure = passivate_edges(
            carbon_scaffold, ch=C_H, min_sep=MIN_SEP, inplane=True
        )

    elif STRUCTURE_TYPE == "flat" and PASSIVATE_FLAT:
        print("[INFO] Passivating flat graphene edges...")
        final_structure = passivate_edges(
            carbon_scaffold, ch=C_H, min_sep=MIN_SEP, inplane=True
        )
        ...
        if STRUCTURE_TYPE == "flat":
            print("[INFO] Flat graphene will NOT be passivated.")
            print("[WARNING] This flat sheet is a finite flake with open edges.")
            print("[WARNING] Use it as a frozen substrate only; do NOT AIREBO-relax it.")
        else:
            print("[INFO] No passivation applied.")

    geo_data = make_geo(final_structure)
    if check_overlaps(geo_data, cutoff=0.90):
        print("[WARNING] Close atomic contacts detected. Tune MIN_SEP or geometry.")
